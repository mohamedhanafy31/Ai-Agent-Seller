...
class EmbeddingComputer:
    def __init__(self, dataset, test_dataset, grid_off, max_batch=1024):
        ...
        os.makedirs("backend/assets/cache/embeddings/", exist_ok=True)
        self.cache_path = "backend/assets/cache/embeddings/{}_embedding.pkl"
        self.cache = {}
        self.cache_name = ""
    ...
    def initialize_model(self):
        if self.dataset == "mot17":
            ...
            if self.test_dataset:
                path = "external/weights/bytetrack_x_mot20.tar"
            else:
                return self._get_general_model()
    ...
    def _get_general_model(self):
        # Initialize base model (without extra layers)
        ...
        # Load checkpoint
        sd = torch.load("backend/assets/models/fine_tuned_model_epoch_32.pth", map_location="cuda")
        # Filter out projection/classifier weights
        new_state_dict = OrderedDict()
        for k, v in sd.items():
            # Remove prefixes and skip proj/classifier layers
            k = k.replace("module.", "").replace("base_model.", "")
            if k.startswith("proj") or k.startswith("classifier"):
